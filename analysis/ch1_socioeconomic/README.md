# Chapter 1 — Socioeconomic deprivation and SARS-CoV-2 transmission clustering in Scotland
...
Clusters whose members predominantly reside in more deprived data zones are larger, less likely to be singletons, and have a broader geographic footprint than clusters drawn from less deprived areas, after adjusting for surveillance intensity (`wn_prop_sequenced`), temporarity (`wn_mid_date`) and lineage (`who_voc`). We expect the magnitude of the deprivation gradient to be strongest during Alpha and Delta and to attenuate during Omicron, when widespread population infection saturated the deprivation signal.
...
One panel per VOC epoch. Each panel shows, from a logistic GLM that also adjusts for the standardised log-sequencing-proportion, the **OR of being a singleton** (vs. a multi-member cluster) for Q1–Q4 relative to **Q5** (least-deprived, not plotted because it is the reference). X-axis has a vertical dashed line at OR = 1. **OR < 1 for Q1 ⇒ the most deprived quintile is *less* likely to be a singleton, i.e. their sequences are more likely to sit inside a genetically linked onward chain** — the epidemiologically interesting direction for Chapter 1. Epochs where Q1's point lies close to 1 with a wide CI are consistent with the hypothesis that widespread population infection (Omicron era) attenuates the deprivation gradient. Companion table `tables/fig4_singleton_ors.csv`: `epoch`, `quintile`, `estimate` (OR vs Q5), `conf_low`/`conf_high`, `p_value`, `z`, `std_error`.
...
python -m manuscripts.paper1_socioeconomic.make_figures --figures manuscripts/paper1_socioeconomic/figures

# Or one at a time:
python -m manuscripts.paper1_socioeconomic.scripts.fig1_sequences_by_simd_over_time
```

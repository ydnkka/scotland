#!/usr/bin/env python3
"""
Process one (window, lineage) group: tn93 CSV → EpiLink weights → Leiden clusters.
...
Usage:
  python3 scripts/03_process_group.py \\
...
Typically called in parallel via scripts/parallel_run.sh with the command file
generated by 05_gen_cluster_commands.py (or directly from the tn93 command file
...
from epilink import estimate_linkage_probabilities
...
def process_group(
    ...
    # Epilink linkage probabilities (edge weights)
    try:
        df["weight"] = estimate_linkage_probabilities(
    ...
    if df.empty:
        logging.warning("%s: all weights < 1e-6 → fallback isolates", stem)
        result = fallback_isolates(group_ids, resolutions, window_id, lineage)
        result.to_parquet(out_path, index=False, compression="zstd")
        return out_path

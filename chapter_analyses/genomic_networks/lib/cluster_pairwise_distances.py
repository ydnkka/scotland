...
from .io import load_analysis_columns, load_pairwise_edges
from .windows import normalise_window, normalise_windows
...
def _load_sequence_rows(
    *,
    ...
    df["window_id"] = df["window_id"].map(normalise_window)

    if windows is not None:
        df = df.loc[df["window_id"].isin(set(normalise_windows(windows) or []))]
    if pango_lineages:
...
def _summarise_one_group(
    ...
    *,
    ...
    base["n_pairwise_rows"] = len(selected_edges)
...
def build_cluster_pairwise_distance_selection(
    *,
    windows: Sequence[str] | None,
    ...
    """Return selected window-lineage groups and sequence-to-cluster maps."""
    ...
    return groups, cluster_by_sequence
...
def build_cluster_pairwise_distance_summary(
    *,
    ...
    groups, cluster_by_sequence = build_cluster_pairwise_distance_selection(
        windows=windows,
        pango_lineages=pango_lineages,
        max_windows=max_windows,
        max_groups=max_groups,

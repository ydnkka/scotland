...
**Background:** SARS-CoV-2 genomic surveillance can reconstruct recent-transmission structure at national scale, but inferred clusters are shaped by both transmission and ascertainment. We examined whether area deprivation was associated with the size, geographic spread, and sociodemographic composition of genomic clusters in Scotland.

**Methods:** We analysed 281,320 SARS-CoV-2 genomes from Scottish residents sampled between July 2020 and February 2023. Sequences were partitioned by lineage within overlapping three-week windows, scored using the EpiLink process-based compatibility model, and clustered with the Leiden community-detection algorithm. We modelled cluster size and geographic spread using two-part hurdle/zero-truncated negative-binomial models, and within-cluster mixing using observed-minus-expected pairwise discordance for SIMD quintile, age band, sex, and joint sociodemographic profile. Models adjusted for lineage, calendar time, local incidence, sequencing intensity, and test positivity.

**Results:** At the primary Leiden resolution, 193,112 clusters were inferred; 84,067 were non-singletons. Higher mean cluster-level deprivation was not associated with larger clusters. Instead, deprivation showed weak negative associations with non-singleton status, positive cluster size, and unadjusted geographic spread. These associations were small relative to surveillance and epidemic-intensity covariates. Deprivation-related signals were clearer in cluster composition: more deprived clusters showed greater age-band mixing and joint-profile mixing but lower sex mixing. SIMD domains and epidemic waves showed marked heterogeneity. In complementary models, clusters with greater excess mixing across SIMD quintiles and age bands were substantially larger and more geographically dispersed.

**Conclusions:** In this national genomic-surveillance dataset, cluster scale was shaped more strongly by surveillance context than by deprivation alone. Deprivation was most visible in who appeared together within inferred clusters, and the relationship depended on SIMD domain and epidemic wave. Equity-focused genomic epidemiology should therefore model surveillance intensity explicitly and avoid interpreting cluster size as a direct proxy for social transmission burden.

**Keywords:** SARS-CoV-2; genomic epidemiology; Scotland; deprivation; SIMD; genomic surveillance; clustering; EpiLink.
...
We analysed SARS-CoV-2 whole-genome sequences collected from Scottish residents between July 2020 and February 2023 through Public Health Scotland and the COVID-19 Genomics UK (COG-UK) surveillance infrastructure [1,6]. Consensus genomes were aligned to the SARS-CoV-2 reference genome (GenBank accession MN908947) and quality-controlled using Nextclade [42]; only records passing the overall Nextclade quality filter were retained. Sequence records were linked to collection date, anonymised patient identifier, residential datazone, age band, sex, Pango lineage, and the Scottish Index of Multiple Deprivation 2020 version 2 (SIMD). SIMD linkage included the overall rank and seven domain ranks: income, employment, education, health, geographic access, crime, and housing [39]. Datazone-level (standard small-area geography of Scotland with $\approx$760 residents on average) testing aggregates and health-board-level surveillance trends were used to derive local surveillance covariates.

### 2.2. Cluster inference pipeline

Cluster inference followed the EpiLink-Leiden framework described elsewhere [31]. Briefly, sequences were grouped into three-week sliding windows advanced in one-week steps and then partitioned by Pango lineage. Within each window-lineage group containing at least two sequences, pairwise consensus genetic distances were computed under the TN93 substitution model [43] using the standalone TN93 implementation [44] and paired with specimen-collection time differences. Each pair was scored using EpiLink, a process-based compatibility model that evaluates whether the observed genetic and temporal separation is consistent with simulated recent-transmission scenarios under specified assumptions about mutation accumulation, disease progression, and infectiousness [31]. Pairwise compatibility scores were used as graph edge weights, and recent-transmission clusters were inferred using the Leiden community-detection algorithm with a modularity-based objective [36] at the primary resolution $\gamma = 0.3$ (with benchmarked partition stability across $\gamma \in$ [0.1, 0.6]). Sequences in window-lineage groups with fewer than two sequences were assigned to singleton clusters.
...
The primary regression analysis examined deprivation as the exposure. For each count outcome, we used a two-part hurdle formulation to separate the probability of exceeding the structural minimum from the magnitude of the positive excess count [45,46] (see Technical Implementation Appendix, Sections 9-10). The first component modelled whether cluster $c$ exceeded the structural minimum:
...
The ZTNB component used an NB2 negative-binomial parent distribution, with variance function [47,48]:
...
All models used cluster-robust standard errors, clustered by analysis window in the primary analysis to account for dependence induced by overlapping windows [49,50]. A sensitivity analysis instead clustered standard errors by health board.

For the custom ZTNB fits, standard errors were obtained using a sandwich covariance estimator with an observed-information bread and score-based meat [49,50] (see Technical Implementation Appendix, Section 11). Let $\hat{\theta}$ denote the fitted parameter vector and $s_c(\hat{\theta})$ the observation-level score. For robust cluster $g$, the cluster-level score was:
...
After applying Nextclade QC and the primary Leiden resolution ($\gamma$ = 0.3), the analysis dataset comprised 789,347 sequence-rows representing 281,320 unique sequences from Scottish residents, distributed across 134 sliding three-week windows and 788 raw Pango lineages (183 levels after rare-lineage pooling). The cluster table contained 193,112 inferred genomic clusters, of which 84,067 (43.5%) were non-singletons and were retained for mixing analyses. Outcome distributions were strongly right-skewed: median cluster size was 1 with 56.5% singletons and a maximum of 2,792 sequences; median distinct datazones was 1 with 61.7% confined to a single datazone and a maximum of 2,100 datazones (Supplementary Figure 1; `figures/supp_fig1_outcome_distributions.png`). Among non-singleton clusters, median size was 3 and median distinct datazones was 3. Wave-level summaries showed a marked shift towards smaller, more spatially confined clusters in late Omicron subwaves (Table 1).
...
In the primary pooled hurdle/ZTNB models, there was little evidence that clusters with higher mean SIMD deprivation were larger or more geographically dispersed (Figure 1 A-D). Higher cluster-level deprivation was associated with slightly lower odds of being non-singleton (OR = 0.971, 95% CI 0.960-0.983, p < 0.001) and with smaller positive cluster sizes among non-singleton clusters (count ratio = 0.926, 95% CI 0.869-0.987, p = 0.018). For geographic spread, the hurdle component was close to the null (OR = 1.004, 95% CI 0.992-1.016, p = 0.522), whereas the positive-count component was significantly below the null (count ratio = 0.851, 95% CI 0.792-0.915, p < 0.001). Thus, among clusters that spanned more than one datazone, clusters with higher mean deprivation tended to span fewer additional datazones.

![Figure 1](figures/fig1_deprivation_overall.png)

However, this negative association appeared to be explained by cluster size. When the positive geographic-spread model was additionally adjusted for log cluster size, the deprivation estimate reversed direction (count ratio = 1.027, 95% CI 1.019-1.035, p < 0.001; Supplementary Figure 8; `figures/supp_fig8_deprivation_size_adjusted.png`). This suggests that the unadjusted negative association between deprivation and geographic spread was driven primarily by the tendency for more deprived clusters to be smaller, rather than by evidence of reduced spatial diffusion among clusters of comparable size.

Surveillance and epidemic-intensity covariates showed substantially stronger associations with cluster scale than deprivation. Per 1 SD increase in each covariate, positive cluster size was most strongly associated with local sequencing fraction (count ratio = 3.240), test positivity (2.649), local incidence (1.650), and window-level sequencing proportion (1.314). A similar pattern was observed for positive geographic spread, with count ratios of 2.269 for local sequencing fraction, 2.999 for test positivity, 1.699 for local incidence, and 1.274 for window-level sequencing proportion. These effects are consistent with the expectation that observed genomic cluster size and spatial extent are shaped not only by transmission dynamics, but also by epidemic intensity and sequencing coverage [51,52].

After adjustment for log cluster size, only local incidence retained a positive association with geographic spread, although the effect was substantially attenuated (count ratio = 1.065). In contrast, local sequencing fraction (0.936), test positivity (0.901), and window-level sequencing proportion (0.938) were negatively associated with size-adjusted geographic spread. This indicates that these covariates were primarily associated with the number of sampled cases in a cluster, rather than with greater geographic dispersion conditional on cluster size.
...
Observed-minus-expected pair-probability matrices showed non-random sociodemographic composition within genomic clusters (Supplementary Figure 3; `figures/supp_fig3_observed_expected_matrices.png`). For SIMD quintile, same-quintile excess mixing was strongest in the most deprived quintile, where pairs of two most-deprived-quintile cases were observed 0.28 pp more often than expected. Same-quintile pairings in quintiles 2 and 3 were also modestly above expectation, whereas pairings involving the least deprived quintile were generally below expectation, including least-deprived-quintile pairs (-0.09 pp). For age, excess pairing was concentrated among young adults: pairs of two cases aged 20-24 years showed the largest positive excess (+0.21 pp), with adjacent 15-19 by 20-24 and 20-24 by 25-29 pairings also above expectation. These patterns indicate that reconstructed clusters retain measurable socioeconomic and age structure beyond that expected from the background lineage-window composition [37,38], but they should be interpreted as sampled genomic-cluster composition rather than direct contact or transmission matrices.
...
**Figure 4.** Excess mixing as predictor: wave-specific effects on cluster scale (ZTNB components). Per-wave adjusted count ratios of the four excess-mixing scores on the two ZTNB count-model components, for the eight dominant Pango-lineage waves. Two panels: (A) positive cluster size (n = 84,067) and (B) positive geographic spread (n = 74,010). Cells are coloured on a shared symmetric ratio scale (capped at ratio 5) and annotated with the count ratio per 1 SD higher excess-mixing score. The corresponding geographic-spread hurdle component is reported in Supplementary Table 1 rather than as a heatmap: among non-singleton clusters, the multiple-datazone outcome is positive for 88.0% of observations and combines with the strong SIMD-quintile excess-mixing predictor to drive the binomial logistic component toward quasi-separation in some waves. This produces implausibly large odds ratios (approximately 29,000 in Alpha) that obscure heatmap interpretation; the overall SIMD value of OR 22.107 (95% CI 18.977-25.753) is shown in Figure 3B. Positive SIMD-quintile and age-band mixing-predictor effects on the ZTNB components are directionally stable across waves but largest in Alpha, Delta, BA.1, and BA.2; late Omicron subwaves (BA.4, BA.5, BQ.1) show attenuated and noisier estimates. `figures/fig4_mixing_wave_specific.pdf`

...
def _prepare_category_mix(category_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    if category_col not in df.columns:
        raise ValueError(
            "Vaccination category/signature summary needs `sse_signature` or "
            "`sse_category`."
        )
...
def plot_vaccination_report(
    ...
    signature_summary: pd.DataFrame | str | Path | Any,
    ...
    *,
    ...
    category_mix = _prepare_category_mix(_read_table(signature_summary))
...
def make_vaccination_figures(
    *,
    ...
    fig = plot_vaccination_report(
        ...
        results_path / "vaccination_mixing_age_conditional_signature_summary.csv",

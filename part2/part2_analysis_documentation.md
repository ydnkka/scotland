...
The Part 2 sequence-level dataset contains 281,320 unique sequences passing
the primary QC filter, representing 193,160 inferred genomic clusters at
Leiden resolution 0.3 — 109,080 singletons (56.5%) and 84,080 non-singletons
(43.5%).
...
## 6. Key Supplementary Results

### 6.1 SIMD domain vaccination gradients (Q5 minus Q1)

Supplementary gradients computed from
`supp_simd_domain_vaccination_gradients.csv`, reporting the percentage-point
(pp) difference in vaccination proportion between the least-deprived quintile
(Q5) and the most-deprived quintile (Q1) among non-singleton clusters, by
wave. Positive values indicate Q5 is more vaccinated; negative values indicate
Q1 is more vaccinated (unusual for dose recency, where higher deprivation can
coincide with earlier booster receipt in some waves).

**Overall SIMD gradient in vaccination proportion (Q5 minus Q1, pp):**

| Wave    | Q5 − Q1 vaccination pp | Q5 − Q1 booster among vaccinated pp | Q5 − Q1 days since last dose |
|---------|------------------------:|-------------------------------------:|-----------------------------:|
| B.1.177 |                   +0.30 |                                  0.0 |                        −12.6 |
| Alpha   |                   +0.28 |                                  0.0 |                         −1.3 |
| Delta   |                   +3.88 |                                 +2.00|                         −3.7 |
| BA.1    |                   +8.05 |                                +17.10|                        −24.2 |
| BA.2    |                  +10.45 |                                +19.77|                        −31.5 |
| BA.4    |                   +3.08 |                                 +7.72|                        −25.6 |
| BA.5    |                   +9.20 |                                +14.26|                        −49.6 |
| BQ.1    |                   +7.52 |                                +17.89|                       −102.6 |
| XBB     |                    0.00 |                                +16.67|                       −218.8 |

The socioeconomic gradient in vaccination proportion was near-zero during the
early waves (B.1.177, Alpha) when vaccination was not yet widely available. The
gradient widened substantially during BA.1 and BA.2 (Q5 more vaccinated by
approximately 8–10 pp), then remained substantial through BA.5 and BQ.1.

The dose-recency gradient (days since last dose, Q5 minus Q1) is consistently
negative in most waves, meaning the most-deprived quintile received their
most-recent dose closer in time to their infection episode. This counterintuitive
pattern reflects the timing of booster campaigns, which reached less-deprived
groups slightly earlier.

**Geographic access domain is a consistent exception.** The Q5-minus-Q1
vaccination gradient for the access domain is negative or near-zero in most
waves, indicating that the rural/remote (lower-access) areas that dominate Q1
on the access scale had vaccination patterns distinct from those of
income/employment/education deprivation. For BA.5 and BQ.1, the access gradient
is markedly negative (−3.3 and −13.8 pp respectively), suggesting that in late
Omicron subwaves, sequenced cases in geographically accessible areas were less
likely to be vaccinated than cases in less-accessible areas.

### 6.2 Booster coverage gradient by SIMD domain

The Q5-minus-Q1 booster-among-vaccinated gradient was absent during B.1.177
and Alpha (before booster campaigns) and emerged during Delta (+2.0 pp for
overall SIMD). By BA.1 and BA.2 it reached +17–20 pp for the income,
education, health, and housing domains. The access domain consistently shows
smaller or negative gradients, as noted above.

### 6.3 Vaccination-status missingness

Vaccination-status completeness varies by wave, age group, SIMD quintile, and
sequencing fraction. Summary data are in
`supp_vaccination_missingness_summary.csv`. Key patterns from this table should
be considered when interpreting vaccination-proportion estimates: missing status
is most common in B.1.177 and early Alpha (before vaccination records were
linked to sequencing data) and varies by health board.

---

## 7. Outputs

### 7.1 Cluster characterisation tables (part2/tables/)
...
### 7.2 Cluster category tables (part2/tables/)
...
### 7.3 Supplementary question tables (part2/tables/)
...
### 7.4 Parquet caches (part2/cache/)
...
### 7.5 Manuscript figures (part2/manuscript/figures/)

Ten figures generated by `part2/manuscript/make_figures.py`. See `README.md`
for the full figure list. Figures are written as PDF, PNG, and LZW-compressed
TIFF at 600 dpi.

---

## 10. Current Main Interpretation

The current Part 2 results support four linked descriptive conclusions:

1. **Vaccination profiles of clusters changed markedly across epidemic waves.**
   All-vaccinated clusters rose from 0.5% in B.1.177 to 80.6% in XBB. None-
   vaccinated clusters fell from 89.5% in B.1.177 to near-zero in BQ.1 and
   XBB. This reflects the sequential rollout rather than differential
   transmission advantage by vaccination status.

2. **Vaccination-status mixing within clusters peaked during Delta and BA.1.**
   Mixed-vaccination clusters were most common during Delta (49.1% of non-
   singleton clusters) and BA.1 (40.6%), when the rollout had reached the
   general adult population but was still heterogeneous across age groups and
   social strata. Homogeneous clusters (most members sharing the same
   vaccination status) became increasingly dominant from BA.2 onwards as
   all-vaccinated clusters dominated.

3. **A consistent socioeconomic gradient in vaccination emerged from Delta
   onwards.** The most-deprived quintile (Q1) had a lower proportion of
   vaccinated cluster members than the least-deprived quintile (Q5) from
   the Delta wave onwards. The gradient was largest during BA.2 (approximately
   10.5 pp for overall SIMD). Booster coverage showed a similar pattern, with
   Q5-minus-Q1 differences of 17–20 pp in BA.1 and BA.2 for most SIMD domains.
   The geographic access domain is a consistent exception: rural/remote areas
   do not show the same deprivation-vaccination alignment as income, employment,
   education, and housing domains.

4. **Dose recency increased monotonically across waves.** Median days since
   last dose among vaccinated cluster members rose from 13 days in B.1.177 to
   218 days in XBB, reflecting that most Omicron-era breakthrough infections
   occurred far beyond the most recent booster dose. The Q5-minus-Q1 dose-
   recency gradient was consistently negative (most-deprived quintile had doses
   closer in time to their infection), which reflects timing of booster campaign
   rollout rather than protection differences.

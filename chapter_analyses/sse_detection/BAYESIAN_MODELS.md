# Bayesian SSE Regression Models <!-- omit in toc -->
...
```text
chapter_analyses/sse_detection/results/sse_outputs/cluster_table.parquet
chapter_analyses/sse_detection/results/sse_outputs/edge_table.parquet
```

The detector also writes descriptive transition tables under the same directory:
...
Those transition summary tables are not regression inputs. The Bayesian workflow depends on the regression-facing columns in `cluster_table.parquet` and the graph edge table above.

These are loaded with:

```python
from chapter_analyses.sse_detection.lib import load_sse_outputs, prepare_regression_data

sse_outputs = load_sse_outputs("chapter_analyses/sse_detection/results/sse_outputs")
...
The definitive predictor definitions live in `chapter_analyses/sse_detection/lib/model/prep.py`.
...
```python
import pandas as pd
from chapter_analyses.sse_detection.lib import default_model_specs
...
`fit.log` is a structured fit report written by the runner. Bambi/PyMC backend stdout and stderr are not written to `fit.log`; by default they are suppressed. Use `--live-progress` when you want sampler progress and backend messages shown live in the terminal during fitting.
...
The standard CSV, parquet, and NetCDF outputs are written via temporary files and then atomically replaced. This prevents downstream readers from seeing partially written output files.
...
1. Edit predictor constants and `default_model_specs(...)` in `chapter_analyses/sse_detection/lib/model/prep.py`.
